...
ブランドメッセージは、広告主のマーケティング受信同意（以下、マス動）会員を対象に、KakaoTalkチャンネルの友だち登録の有無に関わらず広告メッセージを送信できるKakaoTalk Bizmessage商品です。

* チャンネルの友だちだけでなく、マス動ユーザーのうち非友だちにも広告メッセージを送信できます。
* ブランドメッセージは広告メッセージのみ送信できます。
* LMS/MMSより低コストで、テキストおよび画像を含めて送信できます。
* テンプレート送信および自由形式メッセージ送信が可能です。
* ブランドメッセージの送信失敗時に、SMSへの代替送信が可能です。
...
ブランドメッセージは、送信対象のタイプに応じて、顧客対象送信と友だち対象送信に分類されます。

<a id="customer-targeted-delivery"></a>

### 顧客対象送信

顧客対象送信は、広告主のマスドキュメントユーザーを対象にメッセージを送信します。顧客対象送信を使用するには、事前に**顧客対象送信使用申請**を完了する必要があります。発信プロファイルへの 080 受信拒否番号の登録が必要です。ターゲティングタイプに応じて、送信可能な対象が異なります。

顧客対象送信使用申請の詳細については、**コンソール使用ガイド** > **発信情報** > **顧客対象送信使用申請**を参照してください。

* **M**: マーケティング受信同意ユーザー全体を対象に広告メールを送信します。受信者がチャンネル友だちでない場合、無料受信拒否情報を含むチャンネルメッセージとして送信されます。
...
* チャンネル友だち数 5 万以上
* 3 か月以内のお知らせトーク送信成功履歴の保有

<a id="friend-targeted-delivery"></a>

### 友だち対象送信

友だち対象送信は、KakaoTalk チャンネル友だち全体を対象にメッセージを送信します。080 受信拒否番号の登録は不要です。

<a id="register-080-opt-out-number"></a>

## 080受信拒否登録

顧客向け送信（ターゲティング M、N、O）の際、発信プロフィールに 080 受信拒否番号を登録する必要があります。

* 080 受信拒否番号は、同一の KakaoTalk チャンネルに属する他の組織、他のプロジェクト、他のディーラーの発信プロフィール全体に一括適用されます。
...
ブランドメッセージは広告メールであるため、次の事項を遵守する必要があります。

* 広告性の内容が含まれるメッセージの先頭に（広告）を表示する必要があります。
* 送信者の名称および連絡先は、メッセージ本文より上部に記載する必要があります。
* メッセージに受信拒否および受信同意の撤回を容易に行えるための措置と方法を具体的に明記する必要があります。
* 受信拒否のために別途追加の操作（ログインなど）が必要な場合、受信拒否を困難にするものとして法令違反に該当します。
