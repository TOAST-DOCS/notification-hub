...
ブランドメッセージは、広告主のマーケティング受信同意（以下、マス同）会員を対象に、カカオトークチャンネルの友だち登録の有無にかかわらず、広告性メッセージを送信できる KakaoTalk Bizmessage の商品です。

* チャンネルの友だちだけでなく、マス同ユーザーのうち非友だちにも広告性メッセージを送信できます。
* ブランドメッセージは広告性メッセージのみ送信できます。
* LMS/MMS よりも低コストで、テキストおよび画像を含めて送信できます。
* テンプレート送信および自由形式での送信が可能です。
* ブランドメッセージの送信に失敗した場合、SMS への代替送信が可能です。
...
ブランドメッセージは、送信対象の種類に応じて、顧客対象送信と友だち対象送信に分かれます。

<a id="customer-targeted-delivery"></a>

### 顧客対象送信

顧客対象送信は、広告主の自社ユーザーを対象にメッセージを送信します。顧客対象送信を使用するには、事前に**顧客対象送信使用申請**を完了する必要があります。発信プロファイルに080受信拒否番号の登録が必要です。ターゲティングの種類によって、送信可能な対象が異なります。
...
友だち対象送信は、KakaoTalkチャンネルの友だち全員を対象にメッセージを送信します。080受信拒否番号の登録は必要ありません。
...
* 080 受信拒否番号は、同一の KakaoTalk チャンネルにおける他の組織、他のプロジェクト、他のディーラーの発信プロフィール全体に一括適用されます。
...
* メッセージに受信拒否および受信同意撤回の意思を容易に行えるための措置と方法を具体的に明記する必要があります。
