...
* [コンソール] お知らせトーク共用テンプレート機能追加
    * 注文、決済、配送など業種を問わず汎用的に使用できるメッセージテンプレートを提供します。別途の審査過程なしに使用できます。
* [コンソール、API] ブランドメッセージチャンネル出시
    * カカオブランドメッセージの送信をサポートします。
    * 送信対象: 顧客会社の会員向け（M、N、Oグループ）、チャンネル友だち向け（既存のIグループターゲティング送信）
    * 既存の KakaoTalk Bizmessage 商品のブランドメッセージとの主な相違点:
        * Oグループターゲティングの追加: 顧客会社の広告性受信同意会員のうち、チャンネル友だち向けへの送信
        * 顧客会社の会員向けメッセージは、カカオトーク 25.4.0 以上のユーザーのみ受信可能
