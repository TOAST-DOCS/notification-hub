...
* [コンソール] お知らせトーク共用テンプレート機能追加
    * 注文、決済、配送などの業種を問わず、汎用的に使用できるメッセージテンプレートを提供します。別途の審査過程なしに使用できます。
* [コンソール、API] ブランドメッセージチャンネルのリリース
    * カカオブランドメッセージの送信をサポートします。
    * 送信対象: 加盟店会員向け（M、N、Oグループ）、チャンネル友だち向け（既存Iグループターゲティング送信）
    * 既存 KakaoTalk BizMessage 商品のブランドメッセージとの主な相違点:
        * Oグループターゲティングの追加: 加盟店の広告性受信同意会員のうち、チャンネル友だちを対象とした送信
        * 加盟店会員向けメッセージは、カカオトーク 25.4.0 以上のユーザーのみ受信可能
