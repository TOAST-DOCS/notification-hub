...
### 機能追加
* [コンソール] お知らせトーク 共用テンプレート機能追加
    * 注文、決済、配送など業種を問わず汎用的に使用可能なメッセージテンプレートを提供します。別途の審査過程なしに使用できます。
* [コンソール、API] ブランドメッセージチャンネルのリリース
    * カカオブランドメッセージの送信をサポートします。
    * 送信対象：顧客企業の会員向け（M、N、O グループ）、チャンネル友だち向け（既存 I グループのターゲティング送信）
    * 既存 KakaoTalk Bizmessage のブランドメッセージとの主な相違点：
        * O グループターゲティングの追加：顧客企業の広告性受信同意会員のうち、チャンネル友だちを対象に送信
