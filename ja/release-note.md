...
* [コンソール、API] ブランドメッセージチャンネルのリリース
    ...
    * 送信対象: 顧客企業会員向け（M、N、Oグループ）、チャンネル友だち向け（既存Iグループターゲティング送信）
    * 既存 KakaoTalk BizMessage 商品のブランドメッセージとの主な相違点:
        * Oグループターゲティングの追加: 顧客企業の広告性受信同意会員のうち、チャンネル友だち向けに送信
...
    * 外部080受信拒否番号の登録/削除、受信拒否番号照会APIが提供されます。詳細は[API v1.0使用ガイド > 発信情報](./api-guide-v1x0/sender-unsubscribe)を参照してください。
