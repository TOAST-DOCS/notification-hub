...
* 添付ファイルアップロードポップアップで添付ファイルを使用するメッセージチャンネルを選択します。
* **ファイル選択**をクリックし、ファイルを選択します。
* **保存**をクリックしてファイルをアップロードします。
* アップロードするファイルが選択したメッセージチャンネルに不適切な場合、アップロードが失敗し、失敗原因が表示されます。

添付ファイルの保存期限は7日間で、テンプレートで使用している添付ファイルは、テンプレートを削除するまで保存されます。
テンプレート登録及びメッセージ送信時にアップロードした添付ファイルも管理できます。


### メッセージチャンネル別添付ファイルの仕様

| メッセージチャンネル | タイプ          | ファイル形式                                            | ファイル最大サイズ | 解像度                        | 比率                       |
| ----------- | ---------------- | ------------------------------------------------------- | ------------- | -------------------------------- | ----------------------------- |
| SMS         | MMS              | .jpg, .jpeg                                             | 300KB         | 横1000px、縦1000px以下 |                               |
| RCS         | MMS              | .jpg, .jpeg, .png, .gif, .bmp                           | 1MB           |                                  |                               |
| お知らせトーク   | 画像        | .jpg, .png                                              | 500KB         | 横500px以上                | (2:1)                         |
| お知らせトーク   | アイテムリスト | .jpg, .png                                              | 500KB         | 横108px以上                | (1:1)                         |
| カカともへのメッセージ   | 画像        | .jpg, .png                                              | 5MB           | 横500px以上                | (2:1)以上(3:4)以下      |
| カカともへのメッセージ   | ワイド画像 | .jpg, .png                                              | 5MB           | 横800px、縦600px           |                               |
| カカともへのメッセージ   | ワイドアイテムリスト | .jpg, .png                                          | 5MB           | 横400px以上800px以下、縦400px |                               |
| カカともへのメッセージ   | カルーセルフィード   | .jpg, .png                                              | 5MB           | 横500px以上                | (2:1)以上(3:4)以下      |
| Email       | -                | .js, .exe, .bat, .cmd, .com, .cpl, .scr, .vbs, .wsr除く全ての形式 | 30MB          |                                  |                               |
