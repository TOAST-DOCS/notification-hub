...
お知らせトーク、ブランドメッセージを送信するには、KakaoTalk発信プロフィールを登録する必要があります。
...
発信プロフィールを登録するには、KakaoTalkチャンネルが作成されている必要があります。KakaoTalkホームページでKakaoTalkチャンネルを作成してください。

* [KakaoTalkチャンネル作成はこちら](https://center-pf.kakao.com/)

発信プロフィールを登録するには、KakaoTalkチャンネル登録後にビジネス認証を受ける必要があります。

発信プロフィールの作成方法については、**利用ポリシーおよび事前設定ガイド > お知らせトーク/ブランドメッセージ**を参照してください。

* [利用ポリシーおよび事前設定ガイド > お知らせトーク/ブランドメッセージはこちら](../service-policy-and-precondition/alimtalk-and-friendtalk)
...
2. 発信プロフィール ID、管理者の携帯電話番号、カテゴリを設定し、**[トークンリクエスト]** をクリックします。
3. 管理者の携帯電話に送信されたトークン番号 6 桁を入力します。
4. **[確認] > [登録]** をクリックすると、発信プロフィールの登録が完了します。

!!! danger "注意事項"
    お知らせトークの1日あたりの**最大送信数のデフォルト値は1,000件**です。1日あたりの最大送信数を変更するには、[カスタマーセンター](https://www.nhncloud.com/kr/support/inquiry)にお問い合わせください。

!!! danger "注意事項"
    ブランドメッセージは**広告性 (AD) メッセージのみ送信可能**です。顧客向け送信（ターゲティング M、N、O）を行う場合、発信プロフィールへの**080受信拒否番号の登録が必須**です。
...
* **[発信プロフィールグループ管理]** をクリックして、グループを作成し発信プロフィールをグループに追加できます。
...
発信プロフィールの詳細画面で **[Kakao統計はこちら]** をクリックすると、新しいウィンドウでKakao統計を照会できます。統計の基準には送信統計とテンプレート統計があり、メッセージチャンネルによって照会条件が異なります。照会結果はチャートと表で確認できます。

* リアルタイム統計は提供されません。前日に収集したデータを毎日午前7時頃に提供します。
* お知らせトークの統計はD+1に初回提供され、D+2に確定します。
* 有効開封数は、同じメッセージに対して重複集計されません。
* クリック数は、同じメッセージに対して重複集計されます。
...
発信プロフィールを基準に、1日あたりの送信数、有効開封数、クリック数を照会します。期間、送信識別子、メッセージタイプなどを設定して照会できます。

<a id="template-statistics"></a>

#### テンプレート統計

テンプレートおよびグループタグを基準に、1日あたりの送信数、有効開封数、クリック数を照会します。期間、メッセージタイプなどを設定して照会できます。
...
1. 発信プロフィールの詳細画面で **[顧客向け送信利用申請]** をクリックします。
2. 広告性情報受信同意の証憑資料ファイルをアップロードします。
    * 証憑資料ファイルはKakaoTalkチャンネル単位で保存されるため、変更時には同一チャンネルの他のディーラー発信プロフィールにも一括適用されます。
    * 他のディーラーがアップロードしたファイルがすでに存在する場合、ファイルアップロードの手順を省略して利用申請が可能です。
3. 利用申請は、以下の条件をすべて満たす場合に承認されます。
    * ビジネス認証チャンネル
    * 事業者番号の登録
