...
2. テンプレートを使用するかどうかを選択します。
    * テンプレートを使用する場合は、送信プロフィールに登録されているテンプレートを選択します。
    * テンプレートを使用しない場合は、メッセージタイプを選択し、内容を作成します。
3. プッシュ通知を使用するかどうかを選択します。
    * プッシュ通知で **[使用しない]** を選択すると、メッセージのプッシュ通知は送信されません。
4. テンプレートを使用しない場合は、メッセージタイプを選択します。
    * **[テキスト]**: 日英を問わずスペースを含む1,300文字のテキスト + リンクボタン最大5個（縦配列）
    * **[イメージ]**: 日英を問わずスペースを含む1,300文字のテキスト + 画像1枚 + リンクボタン最大5個（縦配列）
    * **[ワイドイメージ]**: 日英を問わずスペースを含む76文字のテキスト + 画像1枚 + リンクボタン最大2個
    * **[ワイドアイテムリスト]**: 1つのタイトルに3〜4個のリスト（画像 + アイテム）+ リンクボタン最大2個（横配列）。1番目のアイテムタイトルは25文字、2〜4番目のアイテムタイトルは30文字。
    * **[カルーセルフィード]**: タイトル20文字のテキスト + フレーズ180文字のテキスト + 画像 + リンクボタン2個（横配列）で構成されたアイテム最大6個
    * **[プレミアム動画]**: カカオTVにアップロードされた動画 + ヘッダー20文字のテキスト + フレーズ76文字のテキスト + リンクボタン1個
    * **[コマース]**: 商品価格および割引情報を強調するタイプ。タイトル20文字のテキスト + 付加情報34文字のテキスト + リンクボタン最大2個（横配列）
    * **[カルーセルコマース]**: 商品情報をカタログ形式で構成するタイプ。タイトル30文字のテキスト + 付加情報34文字のテキスト + リンクボタン最大2個（横配列）で構成されたアイテム最大6個。すべての画像は同じ比率である必要があります。
5. 画像がある場合は画像を選択します。画像を添付するには、先に画像を登録する必要があります。
6. ボタンを挿入できます。
7. 送信対象タイプを選択します。
    * **[顧客向け送信]**: 広告主のマーケティング受信同意ユーザーを対象に送信します。080受信拒否番号の登録が必要です。
    * **[友だち向け送信]**: KakaoTalkチャンネルの友だち全員を対象に送信します。
8. 顧客向け送信の場合は、ターゲティングタイプを選択します。
    * **M**: マーケティング受信同意ユーザー全員を対象に広告メッセージを送信します。
    * **N**: マーケティング受信同意ユーザーのうち、チャンネル友だちを除いて広告メッセージを送信します。
    * **O**: マーケティング受信同意ユーザーのうち、チャンネル友だちを対象に広告メッセージを送信します。
9. 顧客向け送信の場合は、080受信拒否番号を設定します。
    * 送信プロフィールに設定された080受信拒否番号を使用するか、直接入力できます。
10. 代替送信メッセージを設定するかどうかを選択します。
    * ブランドメッセージの送信が失敗した場合、SMSで代替送信できます。
11. 受信者を追加します。
...
| ボタン種類 | 説明 |
| --- | --- |
| Webリンク | モバイルまたはPC Webページに移動します。 |
| アプリリンク | カスタムスキームでアプリを起動します。AndroidとiOSのスキームをそれぞれ設定します。 |
| ボットキーワード | ボタン名が担当者に送信されます。 |
| メッセージ転送 | ボタン名、メッセージ本文が担当者に送信されます。 |
