...
#### ブランドメッセージテンプレート
ブランドメッセージテンプレートは、お知らせトークテンプレートと異なり、審査プロセスがなく、作成、修正、削除を自由に行えます。

* 送信プロファイルを選択し、テンプレートを登録します。
* テンプレートコードはユーザーが直接登録する方式ではなく、カカオがランダムな識別子を付与します。
* メッセージタイプを選択し、内容を作成します。
    * サポートするメッセージタイプ: テキスト、画像、ワイド画像、ワイドアイテムリスト、カルーセルフィード、プレミアム動画、コマース、カルーセルコマース
* ボタンを登録できます。
    * サポートするボタンタイプ: Webリンク、アプリリンク、ボットキーワード、メッセージ転送、相談トーク切り替え、ボット切り替え、ビジネスフォーム、チャンネル追加
* クーポンを登録できます。
* 画像を添付するには、先に画像を登録する必要があります。

<a id="public-alim-talk-templates"></a>

#### 共用お知らせトークテンプレート
共用お知らせトークテンプレートは、Kakaoが直接作成・審査して公開したテンプレートです。すべての事業者が共通して使用でき、特定の送信プロフィールに帰属しません。Kakaoの審査がすでに完了した状態で提供されるため、別途審査の申請なしにすぐに送信に使用できます。

* 自動登録
    * 送信プロフィールを登録すると、共用お知らせトークテンプレートが自動的に同期されコンソールに表示されます。その後、Kakaoに新しい共用テンプレートが公開されると、定期的に自動同期されます。
    * 共用お知らせトークテンプレートは、**[Kakao]** カテゴリの下に自動的に分類されます。このカテゴリはシステムが作成・管理するため、ユーザーが直接変更または削除することはできません。
* 使用制限
    * 共用お知らせトークテンプレートはKakaoが管理するため、コンソールで変更または削除することはできません。
    * 共用お知らせトークテンプレートを一般カテゴリに移動したり、一般テンプレートを **[Kakao]** カテゴリに移動したりすることはできません。
* 送信
    * 共用お知らせトークテンプレートは特定の送信プロフィールに帰属しないため、送信時に使用する送信プロフィールを直接選択する必要があります。
    * アクティブ状態の送信プロフィールが1つ以上登録されている必要があります。
    * 審査が完了した状態であるため、審査待ちなしにすぐに送信できます。
