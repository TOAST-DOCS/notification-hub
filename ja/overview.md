...
* 受信者の連絡先（メール、携帯電話番号、トークン）を体系的に管理できます。
    * 受信者をグループで管理できます。
    * 受信者が受信拒否した履歴を管理し、不要なメッセージの送信を防ぐことができます。
...
* すべてのメッセージチャンネルのテンプレートを登録および管理できます。
    * テンプレートを使用することで、繰り返しのメッセージ作成を削減し、一貫したメッセージを簡単に送信できます。
...
* 事前に登録したテンプレートでフローを作成できます。
    * フローを使用して最大 6 つのチャンネルに同時にメッセージを送信できます。端末の状態によりメッセージの受信に失敗した場合は、事前に設定した送信順序に従って次のチャンネルへ自動送信されます。
    * メッセージチャンネルの優先順位設定方法に応じて、受信率の向上や送信コストの削減など、さまざまな目的に活用できます。
