...
* 발신 번호 차단으로 문자 전송에 실패한 경우에는 '번호 도용 문자 차단 서비스'를 확인 바랍니다. 
    * [번호 도용 문자 차단 서비스 가이드 바로가기](./service-policy/service-policy-sms#fraud-number)
* 전송 결과는 성공이지만 문자를 수신하지 못하는 경우는 '통신사 스팸 차단 서비스'를 확인 바랍니다.
    * [통신사 스팸 차단 서비스 가이드 바로가기](./service-policy/service-policy-sms#spam-number)
...
* 메시지 타입을 선택합니다. 타입별 상세 가이드는 [친구톡 발송 지원 타입](https://docs.nhncloud.com/ko/Notification/KakaoTalk%20Bizmessage/ko/friendtalk-overview/#_3)에서 확인하세요.
    * 기본형(텍스트/이미지/와이드 이미지)
...
#### 국제 SMS 메시지 발송 설정
* 국제 SMS 발송 기능 이용 전 반드시 [[국제 SMS 서비스 정책]](./service-policy/service-policy-international-sms)을 확인합니다.
...
* 자동 차단 월 제한 건수 및 임계치 알림
    ...
    * **자동 차단 월 제한 건수**는 보조 기능으로 임계치 초과에 대한 탐지가 실시간으로 반영되지 않을 수 있습니다. 보조 기능의 일부 오차에 대해 NHN Cloud는 책임을 지지 않으니 주의해서 사용하시기 바랍니다.
...
| 메시지 채널 | 통계 기준 | 이벤트 | 비고 |
| - | - | - | - |
| 전체 | 메시지 | 요청됩(REQUESTED), 취소됨(CANCELED), 발송 성공(SENT), 발송 실패(SEND_FAILED), 수신(DELIVERED), 수신 실패(DELIVERY_FAILED) | 발송 과정에서 발생한 이벤트입니다. |
| SMS | 메시지 | 요청됩(REQUESTED), 취소됨(CANCELED), 발송 성공(SENT), 발송 실패(SEND_FAILED), 수신(DELIVERED), 수신 실패(DELIVERY_FAILED) | |
| RCS | 메시지 | 요청됩(REQUESTED), 취소됨(CANCELED), 발송 성공(SENT), 발송 실패(SEND_FAILED), 수신(DELIVERED), 수신 실패(DELIVERY_FAILED) | |
| 알림톡 | 메시지 | 요청됩(REQUESTED), 취소됨(CANCELED), 발송 성공(SENT), 발송 실패(SEND_FAILED), 수신(DELIVERED), 수신 실패(DELIVERY_FAILED) | |
| 친구톡 | 메시지 | 요청됩(REQUESTED), 취소됨(CANCELED), 발송 성공(SENT), 발송 실패(SEND_FAILED), 수신(DELIVERED), 수신 실패(DELIVERY_FAILED) | |
| Push | 메시지 | 요청됩(REQUESTED), 취소됨(CANCELED), 발송 성공(SENT), 발송 실패(SEND_FAILED), 수신(DELIVERED), 수신 실패(DELIVERY_FAILED), 열람됨(OPENED) | 메시지 열람에 대한 이벤트도 수집됩니다. |
| Email | 메시지 | 요청됩(REQUESTED), 취소됨(CANCELED), 발송 성공(SENT), 발송 실패(SEND_FAILED), 수신(DELIVERED), 수신 실패(DELIVERY_FAILED), 열람됨(OPENED) | 메시지 열람에 대한 이벤트도 수집됩니다. |
| SMS | 국제 SMS 메시지 | 요청됩(REQUESTED), 취소됨(CANCELED), 발송 성공(SENT), 발송 실패(SEND_FAILED), 수신(DELIVERED), 수신 실패(DELIVERY_FAILED) | |
