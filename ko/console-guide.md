...
도메인 보호를 비활성화 하는 경우, 다른 프로젝트에서 임의로 도메인을 사용할 수 있습니다. 모든 인증을 완료한 도메인의 경우, 다른 프로젝트에서 발송하는 메일도 동일하게 이메일 수신 서버에서 정상적으로 수신이 됩니다. 이렇게 발송된 이메일이 스팸이거나 피싱인 경우, 수신자에게 피해가 발생할 수 있고 도메인의 평판이 하락해 수신 이메일 서버에서 수신을 거부할 수 있습니다.
...
- 외부
    ...
    - iOS
        - iOS 9 이상에서 HTTP를 사용하려면 Info.plist 파일내에 ATS(App Transport Secuirty)를 설정해야 합니다.
