...
<span id="attachmentV10AttachmentsAttachmentIdDoValidatePost"></span>

## 업로드 된 첨부 파일 유효성 검사

업로드 된 첨부 파일에 대해 새로운 파일 유형의 유효성을 검사합니다. 파일 유형 수정 API 호출 전 파일 유형에 대한 유효성을 검사할 수 있습니다.
...
| 경로 | 타입 | 필수 | 설명 |
| - | - | - | - |
| fileTypes | Array | Y | 첨부파일 유효성 검사를 위한 파일 유형 목록 |
...
<span id="attachmentV10AttachmentsAttachmentIdFileTypesPost"></span>

## 업로드 된 첨부 파일의 파일 유형 수정

업로드 된 첨부 파일의 파일 유형을 수정합니다.
...
| 경로 | 타입 | 필수 | 설명 |
| - | - | - | - |
| fileTypes | Array | N | 개별 상품 첨부파일 업로드시 지정할 파일 타입 목록, 하나 이상 입력 가능. 템플릿 업로드간 사용시 EMAIL은 EMAIL_TEMPLATE, SMS는 SMS_TEMPLATE로 업로드 요청 필요 |
...
<span id="attachmentV10AttachmentsAttachmentIdGet"></span>
...
```
{
  ...
  "attachment" : {
    ...
    "fileSizeByte" : 21391,
    "createDateTime" : "2025-03-21T10:44:40.803+09:00",
    "expireDateTime" : "2025-03-21T10:44:40.804+09:00",
...
| header.resultCode | Integer | 요청의 결과 코드입니다.<br>기본값: 0 |
| header.resultMessage | String | 요청과 메시지입니다.<br>기본값: SUCCESS |
| attachment | Object | No description |
...
| attachment.fileSizeByte | Long | 첨부파일의 사이즈 단위는 byte |
| attachment.createDateTime | String | 파일 업로드 일시 |
| attachment.expireDateTime | String | 파일 만료 일시 |
| attachment.uploadedFileTypes | Array | 개별 상품에 업로드 된 파일 유형 목록 |
...
<span id="attachmentV10AttachmentsDoValidatePost"></span>
...
<span id="attachmentV10AttachmentsFileTypesGet"></span>
...
```
{
  ...
  "attachments" : [ {
    ...
    "fileFormat" : "JPG",
    "fileSizeByte" : 21391,
    "createDateTime" : "2025-03-21T10:44:40.820+09:00",
    "expireDateTime" : "2025-03-21T10:44:40.820+09:00",
    "uploadedFileTypes" : [ "EMAIL_DEFAULT" ]
  } ]
}
```
...
| attachments[].fileSizeByte | Long | 첨부파일의 사이즈 단위는 byte |
| attachments[].createDateTime | String | 파일 업로드 일시 |
| attachments[].expireDateTime | String | 파일 만료 일시 |
| attachments[].uploadedFileTypes | Array | 개별 상품에 업로드 된 파일 유형 목록 |
