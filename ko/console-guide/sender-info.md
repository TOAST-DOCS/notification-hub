...
SMS, LMS, MMS 메시지를 발송하기 위해서는 발신 번호를 등록해야 합니다. 전기통신사업법 관련 고시 준수를 위해 발신 번호 등록 시 발신 번호에 대한 명의자 인증이 필요합니다. 명의자 인증 시 회원 및 발신 번호 유형에 따라 인증 방법과 필요한 서류가 결정됩니다.
...
|---|---|---|---|
| 사업자 회원 | 대표자번호, 사업자 자체 번호 | 서류 인증 | 통신서비스 이용증명원 |
| 사업자 회원 | 임직원 번호 | 서류 인증 | 통신서비스 이용증명원, 재직증명서 |
| 사업자 회원 | 타사 번호 | 서류 인증 | 통신서비스 이용증명원, 이용승낙서, (타사)사업등록증, 사업자와 타사 간 관계 확인 문서(계약서 등) |
...
* 재직증명서는 **발급일이 표기되어 있고, 직인이 날인된 서류**만 가능합니다.
...
1. RCS Biz Center 가입 및 브랜드 생성 및 승인
2. 브랜드 운영 관리 메뉴에서 (주)NHN Cloud를 대행사로 지정
3. 대화방(발신 번호) 등록 및 승인

RCS Biz Center에서의 사전 등록 사항에 대한 자세한 내용은 **이용 정책 및 사전 설정 안내 > RCS**를 참고하세요.
...
SPF(sender policy framework, 발신자 정책 프레임워크)는 이메일 발신자와 발송 서버의 신뢰성을 검증하기 위한 메커니즘으로, 이메일 수신 서버가 특정 도메인에서 발송된 메일이 실제로 허가된 이메일 발송 서버에서 왔는지 확인합니다.
...
DKIM(domainkeys identified mail, 도메인 키 식별 메일)은 이메일 발송 서버가 이메일을 디지털 서명하고 이메일 수신 서버는 발신자 진위 여부를 확인하여 전송 중에 메시지가 위조, 변조되지 않았는지 확인하는 이메일 검증 방법입니다.
...
DMARC(domain-based message authentication reporting and conformance)는 이메일 보안 강화의 마지막 단계입니다. 이메일 스푸핑을 이용한 피싱, 사기 등을 막기 위한 도메인 기반 메시지 인증에 대한 보고 및 준수 정책입니다.
...
Push 메시지를 발송하기 위해서는 푸시 서비스에서 발급하는 인증 정보를 등록해야 합니다.
...
Android 기기에 푸시 메시지를 발송하기 위해서는 FCM(Firebase Cloud Messaging) 인증 설정이 필요합니다.
...
iPhone에 푸시 메시지를 발송하기 위해서는 APNS(Apple Push Notification Service) 인증 설정이 필요합니다.
...
Amazon Kindle, Fire 등의 기기에 푸시 메시지를 발송하기 위해서는 ADM(Amazon Device Messaging) 인증 설정이 필요합니다.
...
알림톡, 친구톡 메시지를 발송하기 위해서는 카카오톡 발신 프로필을 등록해야 합니다.
...
발신 프로필 생성에 대한 자세한 내용은 **이용 정책 및 사전 설정 안내 > 알림톡/친구톡**을 참고하세요.

* [이용 정책 및 사전 설정 안내 > 알림톡/친구톡 바로 가기](../service-policy-and-precondition/alimtalk-and-friendtalk)
...
1. **+ 발신 프로필 등록**을 클릭합니다.
2. 발신 프로필 아이디, 관리자 휴대폰 번호, 카테고리를 설정한 뒤 **토큰 요청**을 클릭합니다.
3. 관리자 휴대폰으로 전송된 토큰 번호 6자리를 입력합니다.
4. **확인 > 등록**을 클릭하면 발신 프로필 등록이 완료됩니다.

!!! danger "주의"
    알림톡 일별 **최대 발송량은 1,000건으로 기본 설정**되어 있습니다. 일별 최대 발송량을 변경하려면 [고객 센터](https://www.nhncloud.com/kr/support/inquiry)로 문의하세요.
...
* 등록된 080 수신 거부 번호를 체크박스로 선택하고 **080 수신 거부 번호 이용 해지**를 클릭합니다.
* '사용 중 - 외부 등록, 사용 중 - 공유 번호' 상태인 080 번호는 이용 해지가 불가합니다.
