...
* 카카오톡 템플릿 코드
    * **카카오톡 템플릿 코드**는 카카오에 등록되는 카카오톡 알림톡 템플릿(이하 카카오톡 템플릿)을 식별하는 고유 코드로 **템플릿 코드**와는 별도 개념입니다.
    * 하나의 템플릿 코드 아래에는 여러 개의 **카카오톡 템플릿 코드(버전)** 이 존재할 수 있습니다. 예를 들어, PAY_NOTICE 템플릿 코드가 PAY_NOTICE_V1, PAY_NOTICE_v2 등 여러 카카오톡 템플릿 코드로 누적될 수 있습니다.
    * 따라서 카카오톡 템플릿 코드를 일종의 버전으로 인식하고 관리하는 것을 권장합니다.
* 카카오 알림톡 템플릿 정책 및 한계
    * 카카오 정책상 **승인이 완료된 카카오톡 템플릿은 수정이 불가**합니다.
    * **검수 중이거나 반려된** 카카오톡 템플릿만 동일한 카카오톡 템플릿 코드를 유지한 채 내용 수정 후 재검수 요청이 가능합니다.
    * 카카오톡 템플릿 코드를 변경하여 신규 등록할 때마다, 해당 카카오톡 템플릿에 대한 **카카오 검수 절차가 새로 진행**됩니다.
* 본 서비스의 알림톡 템플릿 수정 방식
    * 본 서비스에서는 사용자 편의를 위해 승인 완료된 알림톡 템플릿에 대한 수정 기능을 제공합니다.
    * 알림톡 템플릿 수정 시, 기존 카카오톡 템플릿 기반으로 **신규 카카오톡 템플릿을 생성**하여 카카오에 등록하고, 신규 카카오톡 템플릿이 승인되기 전까지는 기존 카카오톡 템플릿이 발송에 사용됩니다.
    * 이 과정에서 새로운 카카오톡 템플릿 생성이 필요한 경우 사용자는 직접 카카오톡 템플릿 코드를 입력해야 합니다.
    * 알림톡 템플릿 수정 후에도 **템플릿 코드는 유지**되므로 사용자는 동일한 템플릿 코드로 계속 발송 할 수 있습니다.
    * 즉, 사용자 입장에서는 '수정'처럼 보이지만, 실제로는 **신규 카카오톡 템플릿 등록(카카오톡 템플릿 코드 교체) + 템플릿 코드 유지** 방식으로 동작합니다.
