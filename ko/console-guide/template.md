...
* 템플릿 코드/템플릿 이름
    ...
    * **템플릿 코드는 사용자가 식별하기 쉬운 별칭**으로, 카카오톡 템플릿 코드와는 별도 개념입니다.
    * 발송시에는 최신 카카오톡 템플릿과 자동 매핑되어 사용됩니다.
* 카카오톡 템플릿 코드 및 수정방식
    * 카카오 정책상 템플릿 수정 기능은 제공되지 않습니다. 하지만 본 서비스에서는 사용자 편의를 위해 수정 기능을 제공합니다.
    * 템플릿 수정 시, 기존 템플릿을 기반으로 신규 템플릿을 생성하여 카카오에 등록하고, 신규 템플릿이 승인되면 기존 템플릿과 자동 교체됩니다.
    * 이 과정에서 새로운 카카오톡 템플릿 생성이 필요하여 사용자는 직접 카카오톡 템플릿 코드를 입력해야 합니다.
    * 템플릿 수정 후에도 템플릿 코드는 유지되므로 사용자는 동일한 코드로 계속 발송 할 수 있습니다.
    * 즉, 사용자 입장에서는 '수정'처럼 보이지만, 실제로는 **신규 템플릿 등록(카카오톡 템플릿 코드 교체) + 템플릿 코드 유지** 방식으로 동작합니다.
    ...
    * 카카오톡 템플릿 코드를 변경하여 신규 등록할 때마다, 해당 템플릿에 대한 카카오 검수 절차가 새로 진행됩니다.
* 템플릿 내용
