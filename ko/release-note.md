...
## 2026. 03. 24.
### 기능 추가
* [API, Console] 통합 RCS 메시지 지원
  * 안드로이드, iOS 기기 모두 수신 가능한 통합 RCS 메시지 발송을 지원합니다.
  * 발송 가능한 통합 RCS 메시지 유형은 [이용 정책 및 사전 설정 안내 > RCS](./service-policy-and-precondition/rcs)를 참고하시기 바랍니다.
* [Console] 카카오 비즈센터 통계 제공
  * 카카오 비즈센터에서 제공하는 알림톡, 브랜드 메시지 통계 데이터를 조회할 수 있는 기능을 제공합니다.
* [API, Console] 카카오 비즈센터 그룹태그 관리 기능 제공
  * 카카오 비즈센터에서 제공하는 그룹태그를 관리할 수 있는 기능을 제공합니다.

## 2026. 01. 27.
### 기능 추가
* [API] 알림톡 템플릿 API 신규 제공 및 일부 API 지원 종료
    * 알림톡 템플릿 관리 구조 변경에 따라 일부 기존 템플릿 API가 신규 API로 대체되었습니다. 자세한 내용은 [API v1.0 사용 가이드 > 템플릿](./api-guide-v1x0/template)을 참고하시기 바랍니다.
* [API] 외부 080 수신 거부 외부 번호 관리 API 신규 제공
    * 외부 080 수신 거부 번호 등록/삭제, 수신 거부 번호 조회 API 가 제공됩니다. 자세한 내용은 [API v1.0 사용 가이드 > 발신 정보](./api-guide-v1x0/sender-unsubscribe)를 참고하시기 바랍니다.
* [Console] 발송 요청 본문 JSON 추출 기능
    * 발송 메뉴에서 설정된 정보의 JSON 추출 기능이 제공됩니다. 자세한 내용은 [콘솔 사용 가이드 > 발송](./console-guide/send-a-message)을 참고하시기 바랍니다.
