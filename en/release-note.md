...
* [Console, API] Launched brand message channel
    ...
    * Target recipients: business members (Groups M, N, and O), channel friends (existing Group I targeted sending)
    * Key differences from brand messages in the existing KakaoTalk BizMessage product:
        * Added Group O targeting: sends to channel friends among members who have consented to receive advertising from the business
        * Messages targeting business members can only be received by KakaoTalk 25.4.0 or later users
...
* [API] Added new AlimTalk template APIs and discontinued some existing APIs
    * Due to changes in the AlimTalk template management structure, some existing template APIs have been replaced with new APIs. For more information, see [API v1.0 User Guide > Template](./api-guide-v1x0/template).
...
* [Console] Added send request body JSON export feature
    * A JSON export feature for the settings configured in the sending menu is now available. For more information, see [Console User Guide > Send a Message](./console-guide/send-a-message).
...
* [API, Console] Discontinued Friend Talk service
    * The Friend Talk service was discontinued on Wednesday, December 31, 2025.
    * Friend Talk-related features are no longer available in the console, and the Friend Talk API can no longer be used.
...
### Feature Updates
* [API] Added the `exact` request parameter to the image layout list retrieval API.
    * Previously, specifying a name with the `name` parameter applied a partial match (LIKE) search.
    * Now, setting the `exact` parameter to `true` retrieves only image layouts with an exact name match.
...
### Feature Updates
* [API/Console] Added support for RCS image templates that include a financial compliance notice field
    * Supports integration and sending of image templates that include a financial compliance notice field in the RCS channel.
...
* [API/Console] Added image layout feature.
    * Image layout is a feature for creating personalized MMS attachment images.
...
* [Console] When sending international SMS, you can now check the encoding and actual send count in the delivery detail view.
...
### Added Features
* [API/Console] You can now view various event histories generated by the service in CloudTrail.
...
* [API] Added response fields to the received results by contacts list retrieval API.
...
* [Console] Added support for RCS BizCenter LMS format type
    * You can now send RCS messages in LMS format type.
...
* [API] Added RCS BizCenter brand message statistics integration
    * You can use the message statistics provided by RCS BizCenter by adding a group ID when sending messages.
* [API] Added RCS message pending expiration period setting
    * When sending RCS messages, you can set the timeout (four types) for delivery attempts to a device.
* [API] Added support for RCS BizCenter LMS format type
    * You can now send RCS messages in LMS format type.
    * You can select the LMS format type when creating a template.
...
### Added Features
* [API] Added the final delivery result list retrieval API by contacts.
...
* [Console/API] Added duplicate recipient check feature
    * If there are duplicate contacts in the recipient list, you can check for duplicates before sending to prevent duplicate deliveries.
* [Console] Added flow template preview feature
    * Added a feature to preview templates from multiple channels included in a flow in the flow management menu.
* [Console] Added AlimTalk quick connect preview
    * Modified the AlimTalk channel template preview to display the Quick Connect item.
* [API] Added user custom fields to the send API
    * Added support for including user custom fields in send API requests.
    * For more information, see [API v1.0 User Guide > Message](./api-guide-v1x0/message).
...
* [Console/API] Improved RCS template feature
    * Improved so that templates registered in RCS BizCenter are automatically integrated without re-registration.
        ...
        * RCS BizCenter templates are registered with the chatbot and opt-out number fields left blank. When sending, the most recently registered chatbot is used.
...
* How to use
    * Products released in alpha are available through **Customer Support > Contact Us**.
* [Console] Console released
    * Added Send, Delivery Result, Address Book, Template, Flow, Detailed Settings, Statistics, and Identity Verification tabs.
* [API] Added API v1.0
    * Added API v1.0 for Message, Received Results by Contacts list retrieval, Template, and Flow.