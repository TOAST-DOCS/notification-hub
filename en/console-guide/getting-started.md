...
After activating Notification Hub, you must complete identity verification before you can use it. For more information about identity verification, refer to **Service Policy and Prerequisites > Identity Verification**.
...
To send RCS messages, you must complete brand integration. If the prerequisites have been completed in RCS Biz Center (brand approved), proceed with integration in the NHN Cloud console. For information on creating a brand in RCS Biz Center, see **Guide to Usage Policies and Preparations** > **RCS**.
...
Once brand creation, agency settings, chat room (sender number) registration, and template registration have been completed (approved) in RCS Biz Center, integrate the brand in the console.
...
For more information about SPF, refer to the following documents.
...
For more information about DKIM, refer to the following documents.
...
Domain-based message authentication, reporting and conformance (DMARC) is the final step in enhancing email security. It is a domain-based message authentication, reporting, and conformance policy to prevent phishing and fraud using email spoofing. The email receiving server looks up the DMARC record in the DNS of the sender's From address domain. Based on the policy defined in the DMARC record, the receiving server authenticates incoming emails.
...
For more information about DMARC, refer to the following documents.

* [Go to Introduction to Email Security Features - Domain Protection, DKIM, DMARC](https://meetup.nhncloud.com/posts/248)

##### Domain Protection

A domain with domain protection enabled cannot be used by other projects. To use a protected domain in another project, domain registration and ownership verification must be completed in the same way.
...
* Click **+ Request 080 Opt-Out Number** and enter your company name. The company name you enter is the name announced when a call is made to the 080 opt-out number.
...
#### Set 080 Opt-Out Number When Sending Advertising Messages

* Advertising messages can only be sent when the 080 opt-out number service is active.
* In the **Delivery > SMS** tab, select Advertising as the message purpose to display the 080 opt-out number selection window.
* Click **Apply Selection** to add the required advertising phrases.
* When sending advertising messages, the required advertising phrases must be included in the message body. The rules are as follows:
