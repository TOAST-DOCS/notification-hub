...
After activating Notification Hub, you must complete identity verification before you can use the service. For more information about identity verification, see **Service Policy and Prerequisites > Identity Verification**.
...
3. Attach the required documents for the selected sender number type.

For more information about the sender number pre-registration system, see **Guide to Usage Policies and Preparations > SMS > Sender Number Pre-Registration**.

* [Go to Sender Number Pre-Registration](../service-policy-and-precondition/sms#sender-phone-number-pre-registration)
...
To send RCS messages, you must complete brand integration. If the prerequisites (brand approval) have been completed in RCS Biz Center, proceed with the integration in the NHN Cloud console. For information about creating a brand in RCS Biz Center, see **Guide to Usage Policies and Preparations** > **RCS**.
...
For more information about sending domains and SPF, DKIM, and DMARC, see **Guide to Usage Policies and Preparations > Email**.
...
2. Enter the root domain to register and click **Validate**.
3. If validation is successful, click **Register** to complete the registration.
4. In the domain list, click **Verify** in the domain ownership verification status column.

When domain ownership verification is successful, the domain verification status changes to "Complete."
...
* [Go to Introduction to Email Security Features (SPF)](https://meetup.nhncloud.com/posts/244)
...
* [Go to Introduction to Email Security Features - Domain Protection, DKIM, DMARC](https://meetup.nhncloud.com/posts/248)
...
1. Complete the DMARC TXT record by adding the email address to receive DMARC reports to the value in the **DMARC** section above.
...
A domain with domain protection enabled cannot be used by other projects. To use a protected domain in another project, the domain must go through the same domain registration and ownership verification process.
...
For information about how to obtain Push authentication credentials, see **Guide to Usage Policies and Preparations > Push**.
...
2. Copy and paste the contents of the FCM Service Account Credential file obtained into the service account key (JSON) field.
3. Click **Validate > Save** to complete the configuration.
...
5. Click **Validate > Save** to complete the configuration.
...
Once the sender profile has been created in Kakao Business, register it by following these steps.

1. Click **+ Register Sender Profile**, set the sender profile ID, administrator's mobile phone number, and category, then click **Request Token**.
...
* Click **+ Request 080 Opt-Out Number** and enter the company name. The company name you enter is announced to callers when they dial the 080 opt-out number.
...
* When the service is completely open, you can find the start date and status of service. While the 080 opt-out number service is scheduled for registration or in use, the SMS Service cannot be closed. Service can be closed only after it is canceled. To cancel the service, click **Cancel**.

<a id="set-080-unsubscription-number-when-advertising-texts"></a>

#### Set the 080 Opt-Out Number When Sending Advertising Messages

* Advertising messages can only be sent when the 080 opt-out number service is active.
* On the **Send > SMS** tab, selecting **Advertising** as the message purpose displays the 080 opt-out number selection window.
