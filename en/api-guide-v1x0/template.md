...
## List Kakao Templates for Alim Talk Templates
...
| limit | Query | Number | X | If not set, the default is 20 (maximum 1000) |
| offset | Query | Number | X | If not set, the default is 0 |
...
```
{
  ...
  "templates" : [ {
    ...
    "content" : {
      ...
      "templateAd" : "Add this channel to receive marketing messages and more via KakaoTalk",
      "templateExtra" : "* Due to the nature of real-time reservations, duplicate reservations may occur, and reservations may be canceled if check-in is not possible.\\n* Inquiry: 1234-1234",
...
| templates[].content.templateExtra | String | X | Template extra information (required when template message type is Extra Information or Mixed Purposes); placeholders cannot be used, but URLs can be included |
...
| templates[].content.templateItemHighlight.title | String | O | Item highlight title (No more than 30 characters; no more than 21 characters if a thumbnail image is present) |
| templates[].content.templateItemHighlight.description | String | O | Item highlight description (No more than 19 characters; no more than 13 characters if a thumbnail image is present) |
...
| templates[].content.securityFlag | Boolean | X | Whether the template is a security template (default: false) |
| templates[].content.categoryCode | String | X | Template category code (see the Query Template Categories API; default: 999999) |
...
| templates[].reviewStatus | String | O | REGISTERED: Requested, REQUESTED: Under review, APPROVED: Approved, REJECTED: Rejected<br>[REGISTERED, REQUESTED, APPROVED, REJECTED] |
...
```
{
  ...
  "template" : {
    ...
    "content" : {
      "messageType" : "SMS",
      "title" : "Holiday Business Hours Notice",
...
```
{
  ...
  "content" : {
    ...
    "templateEmphasizeType" : "NONE",
    "templateContent" : "Your order for #{name} has been completed.",
    "templateAd" : "Add this channel and receive marketing messages from this channel via KakaoTalk",
    "templateExtra" : "* Due to the nature of real-time reservations, duplicate bookings may occur, and reservations may be canceled if check-in is unavailable.\\n* Inquiry: 1234-1234",
    "templateTitle" : "123,450 KRW",
    "templateSubtitle" : "Approval details",
    "templateHeader" : "Your order has been placed.",
    "templateItem" : {
...
| content | Object | O |  |
| content.templateMessageType | String | X | Types of template message (BA: Basic, EX: Extra Information, AD: Ad Included, MI: Mixed Purposes, default: BA) |
...
| content.templateAd | String | X | Channel add guide message (fixed value when template message type is Ad Included or Mixed Purposes) |
| content.templateExtra | String | X | Template extra information (required when template message type is Extra Information or Mixed Purposes); placeholder variables cannot be used; URLs can be included |
...
| content.templateSubtitle | String | X | Auxiliary template phrase (No more than 50 characters, Android: To be abbreviated if it exceeds 18 characters, iOS: To be abbreviated if it exceeds 21 characters) |
...
| content.templateItem.summary.description | String | O | Summary description (only variables, currency units, numbers, commas, and periods can be used) |
| content.templateItemHighlight | Object | X |  |
| content.templateItemHighlight.title | String | O | Item highlight title (up to 30 characters; up to 21 characters when a thumbnail image is present) |
| content.templateItemHighlight.description | String | O | Item highlight description (up to 19 characters; up to 13 characters when a thumbnail image is present) |
...
| content.categoryCode | String | X | Template category code (refer to the Query Template Categories API; default: 999999) |
...
### Register an Alim Talk Template
...
X-NHN-Authorization: Bearer {accessToken}
{
  ...
  "content" : {
    ...
    "templateContent" : "Your order for #{name} has been completed.",
    "templateAd" : "Add this channel and receive marketing messages from this channel via KakaoTalk",
    "templateExtra" : "* Due to the nature of real-time reservations, duplicate bookings may occur, and reservations may be canceled if check-in is not possible.\\n* Inquiries: 1234-1234",
    "templateTitle" : "123,450 KRW",
    "templateSubtitle" : "Approval details",
    "templateHeader" : "Your order has been placed.",
    "templateItem" : {
...
| template.content.templateMessageType | String | X | Template message type (BA: Basic, EX: Extra information, AD: Add channel, MI: Mixed, default: BA) |
...
| template.content.templateAd | String | X | Add channel guide message (fixed value when template message type is Add channel or Mixed) |
...
| template.content.securityFlag | Boolean | X | Whether the template is secured (default: false) |
...
| template.content.buttons[].type | String | O | Template button type<br>[WL(Web link), AL(App link), DS(Delivery search), BK(Bot keyword), MD(Message delivery), BC(Bot for Consultation), BT(Bot Transfer), AC(Add channel), BF(Business form), P1(Image secure transmission plugin), P2(Personal information use plugin), P3(One-click payment plugin), TN(Phone call)] |
...
| content.cards[].buttons[].buttonType | String | X | COMPOSE (open chat room), CLIPBOARD (copy), DIALER (make a call), MAP_SHOW (show map), MAP_QUERY (search map), MAP_SHARE (share current location), URL (link to URL), CALENDAR (add event)<br>※ Using a CLIPBOARD (copy) button with a unified message type prevents delivery on iOS devices.<br><br>[COMPOSE, CLIPBOARD, DIALER, MAP_SHOW, MAP_QUERY, MAP_SHARE, URL, CALENDAR] |
...
| template.categoryId | String | X | Category ID |
| template.messageChannel | String | X | Message channel<br>[SMS(SMS), ALIMTALK(Alim Talk), EMAIL(Email), RCS(RCS), PUSH(Push)] |
| template.messagePurpose | String | X | Message content type<br>Default: NORMAL<br>[NORMAL(General), AD(Advertisement), AUTH(Authentication)] |
| template.messagePurposes | Array | X |  |
| template.templateLanguage | String | X | Template language type<br>Default: PLAIN_TEXT<br>[PLAIN_TEXT(Plain text), FREEMARKER(FreeMarker template)] |
| template.sender | Object | X |  |
...
| template.content.cards[].attachmentId | String | X | Attachment file ID<br>※ If a GIF image is attached to a unified MMS card, it cannot be received on iOS devices. |
...
| Name | Division | Type | Required | Description |
| - | - | - | - | - |
| X-NC-APP-KEY | Header | String | O | App key |
| X-NHN-Authorization | Header | String | O | Access token |
| templateId | Path | String | O | Template ID |
...
| content.cards[].description3 | String | X | Body 3 |
| content.cards[].buttons | Array | X | RCS button list |
| content.cards[].buttons[].buttonType | String | X | COMPOSE (open chat room), CLIPBOARD (copy), DIALER (make a call), MAP_SHOW (show map), MAP_QUERY (search map), MAP_SHARE (share current location), URL (link to URL), CALENDAR (add event)<br>※ Using a CLIPBOARD (copy) button with a unified message type makes it impossible to receive on iOS devices.<br><br>[COMPOSE, CLIPBOARD, DIALER, MAP_SHOW, MAP_QUERY, MAP_SHARE, URL, CALENDAR] |
| content.cards[].buttons[].buttonJson | Object | X |  |
| content.cards[].buttons[].buttonJson.action | Object | X | Button action |
| content.buttons | Array | X | (Deprecated, use content.cards[].buttons) RCS button list |
| content.buttons[].buttonType | String | X | COMPOSE (open chat room), CLIPBOARD (copy), DIALER (make a call), MAP_SHOW (show map), MAP_QUERY (search map), MAP_SHARE (share current location), URL (link to URL), CALENDAR (add event)<br>※ Using a CLIPBOARD (copy) button with a unified message type makes it impossible to receive on iOS devices.<br><br>[COMPOSE, CLIPBOARD, DIALER, MAP_SHOW, MAP_QUERY, MAP_SHARE, URL, CALENDAR] |
