...
Brand Messages is a KakaoTalk Bizmessage product that allows you to send advertising messages to members who have consented to receive marketing from an advertiser (hereinafter referred to as "marketing consent"), regardless of whether they are friends of the KakaoTalk channel.
...
Brand messages are divided into customer-targeted delivery and friend-targeted delivery, depending on the recipient type.
...
For more information about applying to use customer-targeted delivery, see **Console User Guide** > **Sender Information** > **Apply to Use Customer-Targeted Delivery**.

* **M**: Sends advertising messages to all users who have agreed to receive marketing messages. If a recipient is not a channel friend, the message is sent as a channel message that includes free opt-out information.
...
* The 080 opt-out number is applied collectively to all sender profiles of other organizations, other projects, and other dealers that use the same KakaoTalk channel.
...
* If a delivery is requested during nighttime hours, it will either be marked as failed or resent after the restriction is lifted, depending on the detailed settings.

## Cautions

Brand messages are advertising messages, so you must comply with the following requirements:

* The message must display "(Ad)" at the beginning of any content that includes advertising.
* The sender's name and contact information must appear above the message body.
* The message must clearly specify the methods and procedures that allow recipients to easily opt out or withdraw their consent to receive messages.
...
* If business verification is revoked, the permission to send non-friend messages (targeting M and N) will also be revoked.
